...
describe("OpenWeather unit tests", function() {
  it("valid response (zip)", async function() {
    const mockFetcher = new MockGetFetcher()
    const endpoint = new OpenWeatherEndpoint(mockFetcher, "none")
    ...
    const response = await endpoint.FetchWeather(new Zip("94040"))
  ...
  it("valid response (city)", async function() {
    ...
    const response = await endpoint.FetchWeather(new City("London"))
    expect(response).toBeDefined()
  ...
  it("invalid city", async function() {
    ...
    const response = await endpoint.FetchWeather(new City("badcity"))
