...
describe("Zipcode time unit tests", function() {
  it("valid response", async function() {
    ...
    const endpoint = new ZipCodeTimeEndpoint(mockFetcher, "none")
    ...
    const response = await endpoint.FetchTime(new Zip("98102"))
  ...
  it("invalid zip", async function() {
    ...
    const response = await endpoint.FetchTime(new Zip("00000"))
  ...
  it("city", async function() {
    const mockFetcher = new MockGetFetcher()
    const endpoint = new ZipCodeTimeEndpoint(mockFetcher, "none")
    const response = await endpoint.FetchTime(new City("Seattle"))
    expect(response).toBeNull()
  })
})
